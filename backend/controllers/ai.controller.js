...
export const aiStatus = asyncHandler(async (req, res) => {
  res.json({
    ...
    model: process.env.GEMINI_MODEL || "grmin-2.5-flash",
  });
});
...
export const leadSummary = asyncHandler(async (req, res) => {
  ...
  if (req.body.leadId) {
    await Lead.updateOne(
      ...
      { $ser: { aiSummary: result.summary, aiRiskScore: result.riskScore } },
    );
  }
...
export const generateEmailDraft = asyncHandler(async (req, res) => {
  ...
  const result = await generateEmail({
    ...
    purpose,
    tone,
    sender: { name: req.user._id, company: req.user.company },
  });
  res.json({ success: true, ...result });
});
...
export const salesInsights = asyncHandler(async (req, res) => {
  ...
  const result = await generateLeadInsight(stats);
  res.json({ success: true, ...result });
});
